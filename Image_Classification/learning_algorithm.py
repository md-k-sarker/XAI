...
class DNN:
    '''
    General Deep Neural Network class, with backpropagation.
    ...
    def __init__(self, classVals, featuresVals, totalNoOfhiddenLayers, noOfhiddenLayerNeurons):
        ...
        weight = np.zeros((HiddenLayerNoOfNeuronWithBias,
                           InputLayerNoOfNeuronWithBias))
        for j in range(0, HiddenLayerNoOfNeuronWithBias):
            weightj = np.float128(np.random.uniform(
                0, 1, size=InputLayerNoOfNeuronWithBias))
            weight[j] = weightj
        ...
        for l in range(1, totalHiddenLayerSize):
            weight = np.zeros((HiddenLayerNoOfNeuronWithBias,
                               HiddenLayerNoOfNeuronWithBias))
            for j in range(0, HiddenLayerNoOfNeuronWithBias):
                weightj = np.float128(np.random.uniform(
                    0, 1, size=HiddenLayerNoOfNeuronWithBias))
                weight[j] = weightj
            self.WEIGHT.append(weight)

        '''assign random weight for hiddenLayer-outputLayer connections'''
        weight = np.zeros(
            (OutputLayerNoOfNeuron, HiddenLayerNoOfNeuronWithBias))
    ...
    def train_model(self, inputVector, outputVector, maxIteration, minError, learningRate=.3, momentum=50, batch=False):
        '''
        ...
        while(iteration < maxIteration and avgCostPerIteration > minError):
            ...
            for i in range(0, inputVector.shape[0]):
                ...
                for j in range(0, self.neuronsInEachLayer[len(self.neuronsInEachLayer) - 1]):
                    costPerInput += -outputVector[i][j] * getLog(OUTPUT[len(OUTPUT) - 1][j]) - (
                        1 - outputVector[i][j]) * (get1MinusThetaLog(OUTPUT[len(OUTPUT) - 1][j]))
                ...
                delta = OUTPUT[len(OUTPUT) - 1] * (1 -
                                                   OUTPUT[len(OUTPUT) - 1]) * error[np.newaxis]
                DELTA.append(delta)
                ...
                if(batch == False):
                    ...
                    for wl, OT, DT in zip(range(len(self.WEIGHT) - 1, -1, -1), reversed(OUTPUT[:len(OUTPUT) - 1]), DELTA):
                        self.WEIGHT[wl] = self.WEIGHT[wl] + \
                            learningRate * OT * DT.T
            ...
            if(iteration % 100 == 0):
                print('IterationNo: ', iteration, ' CostPerIteration: ', avgCostPerIteration,
                      ' ActualOutput[', sampleNo, ']: ', outputVector[sampleNo], ' PredictedOutput: ', finalLayerOutputs[sampleNo])

        return self.WEIGHT, costsPerIteration

    def test_model(self, inputVector, outputVector, Weight):
        '''
